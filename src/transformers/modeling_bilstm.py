...
class biLSTM(BertPreTrainedModel):
    """
    ...
    def __init__(self, config):
        ...
        self.embeddings = nn.Embedding(config.vocab_size, config.embedding_size, padding_idx=config.pad_token_id)
    ...
    Examples::
        ...
        embedding_output = self.input_dropout(embedding_output)
        encoder_outputs, (hn, cn) = self.encoder(embedding_output)

        # hn = hn.view(self.config.num_hidden_layers, 2, -1, self.config.hidden_size)
        # pooled_output = torch.cat([hn[-1, 0], hn[-1, 1]], dim=-1)
        # Max pooling:
        pooled_output = torch.max(encoder_outputs, 1)[0]
        ...
        outputs = (sequence_output, pooled_output,) + (encoder_outputs[
            1:
        ], None)  # add hidden_states and attentions if they are here
...
class biLSTMForSequenceClassification(BertPreTrainedModel):
    def __init__(self, config):
        ...
        self.dense_map_for_sentence_pair = nn.Linear(config.hidden_size * 8, config.hidden_size * 2)

        self.init_weights()

    def set_input_embeddings_weight(self, value):
        self.bilstm.embeddings.weight.data = value
    ...
    Examples::
        ...
        """
        if input_ids.dim() == 3:
            s1_input_ids = input_ids[:, 0]
            s2_input_ids = input_ids[:, 1]
            outputs = self.bilstm(s1_input_ids)
            s1_enc = outputs[1]
            outputs = self.bilstm(s2_input_ids)
            s2_enc = outputs[1]
            rep = self.dense_map_for_sentence_pair(torch.cat([s1_enc, s2_enc, torch.abs(s1_enc - s2_enc), s1_enc * s2_enc], 1))
        else:
            outputs = self.bilstm(input_ids)
            rep = outputs[1]
        ...
        if labels is not None:
            if self.num_labels == 1:
                ...
                if isinstance(labels, tuple):  # Soft + hard label
                    soft_labels, hard_labels = labels
                    soft_labels = F.log_softmax(soft_labels, dim=-1, dtype=torch.float32)
                    log_prob = F.log_softmax(logits, dim=-1, dtype=torch.float32)
                    kld_loss = -torch.exp(soft_labels) * log_prob + torch.exp(soft_labels) * soft_labels
                    kld_loss = torch.mean(torch.sum(kld_loss, dim=-1))
                    loss_fct = CrossEntropyLoss()
                    ce_loss = loss_fct(logits.view(-1, self.num_labels), hard_labels.view(-1))
