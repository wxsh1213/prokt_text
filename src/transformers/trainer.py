...
class Trainer:
    ...
    def __init__(
        ...
        self._eval_record = []
        self._eval_best_index = -1

    def _add_record(self, loss_dict, step):
        self._eval_record.append({"step": step, "loss_dict": loss_dict})
        key_key = "eval_" + self.args.best_checkpoint_metric
        key_values = [x["loss_dict"][key_key] for x in self._eval_record]
        func = max if self.args.maximize_best_checkpoint_metric else min
        best_value = func(key_values)
        best_index = key_values.index(best_value)
        self._eval_best_index = best_index

    def get_best_record(self):
        if self._eval_best_index > -1:
            return self._eval_record[self._eval_best_index]
        else:
            print("No any loss record...")
            return None

    def pack_train_output(self, tr_loss):
        return TrainOutput(self.global_step, tr_loss / self.global_step)

    def training_prepare(self, t_total, model_path=None):
        # 这里把一些对于model, optimizer, scheduler的准备工作拆出来
        ...
        return model, optimizer, scheduler, total_train_batch_size
    ...
    def get_train_dataloader(self, sampler=None, return_sampler=False) -> DataLoader:
    ...
    def output(self, inputs, model):
        ...
        new_inputs["labels"] = (logits, inputs["labels"])
        return new_inputs, outputs

    def _training_step(
        self, model: nn.Module, inputs: Dict[str, torch.Tensor], optimizer: torch.optim.Optimizer, criterion=None,
            **kwargs,
    ) -> float:
        model.train()
        for k, v in inputs.items():
            if isinstance(v, torch.Tensor):
                ...
                to_device = lambda x: x.to(self.args.device)
                inputs[k] = tuple(map(to_device, v)) # v.to(self.args.device)
            else:
                raise NotImplementedError

        outputs = model(**inputs)
        if criterion is not None:
            loss = criterion.forward(outputs, **kwargs)
        else:
        ...
        if self.args.dual_optimization and hasattr(criterion, "get_constraint_loss"):
            constraint_loss=criterion.get_constraint_loss(outputs)
    ...
    def evaluate(
        ...
        self._add_record(loss_dict=output.metrics, step=self.global_step)
