        ...
        for step, inputs in enumerate(epoch_iterator):
            ...
            if trainer_student.args.train_teacher:
                ...
                if trainer_student.args.dual_optimization:
                    student_output = trainer_student.output(inputs_student, model_s)
                    ...
                    if trainer_student.args.move_back_weight > 0.0:
                        student_output, _ = trainer_student.output(inputs_student, model_s)
                        for key in student_output:
                            if key == "labels":
                                continue
                            student_output[key] = inputs_teacher[key]
                        tr_loss_teacher += trainer_teacher._training_step(model_t, student_output, optimizer_t, criterion=criterion_t)
                    else:
                        tr_loss_teacher += trainer_teacher._training_step(model_t, inputs_teacher, optimizer_t, criterion=criterion_t)

                _optim_step(model_t, trainer_teacher, optimizer_t, scheduler_t)


            teacher_output, _ = trainer_teacher.output(inputs_teacher, model_t)
            for key in teacher_output:
                if key == "labels":
                    continue
                teacher_output[key] = inputs_student[key]

            model_s.train()
            tr_loss += trainer_student._training_step(model_s, teacher_output, optimizer_s, criterion=criterion_s)
...
def main():
    ...
    print(config_teacher)

    # Build for student:
    # Not any pre-trained model for LSTM.
    config_student = CONFIG_MAPPING[model_args.model_type]()
    logger.warning("You are instantiating a new config instance from scratch.")
    config_student.num_labels = num_labels
    ...
    data_args.sep_pair = False
    data_args.comb_pair = False
    data_args.overwrite_cache = True
    eval_dataset_bert = GlueDataset(data_args, tokenizer=tokenizer, evaluate=True) if training_args.do_eval else None
    data_args.sep_pair = True
    data_args.comb_pair = False
    data_args.overwrite_cache = True
    eval_dataset_bilstm = GlueDataset(data_args, tokenizer=tokenizer, evaluate=True) if training_args.do_eval else None
    ...
    trainer_student = Trainer(
        ...
        eval_dataset=eval_dataset_bilstm,
    ...
    if training_args.do_eval and training_args.local_rank in [-1, 0]:
        ...
        eval_datasets = [eval_dataset_bilstm]
